...
public protocol GetMoneyCellDelegate {
    ...
    optional func selecetBankCard(button:UIButton)
...
class GetMoneyCell: UITableViewCell,UITextFieldDelegate {
    ...
    @IBAction func selectBankCard(sender: UIButton) {
...
extension getMoneyDetailCell{
    ...
    /**
     ...
     - returns: 是否可输入
     */
    func moneyInput(text: String, range: NSRange, replaceString: String, remain: Int) -> Bool {
        
        var inputText = text
        
        let scanner = NSScanner(string: inputText)
        var numbers:NSCharacterSet
        
        //字符串限制的可输入字符
        let pointRange:NSRange = (inputText as NSString).rangeOfString(".")
        if (pointRange.length > 0) && (pointRange.location < range.location || pointRange.location > range.location + range.length) {
            numbers = NSCharacterSet.init(charactersInString: "0123456789")
        }else{
            numbers = NSCharacterSet.init(charactersInString: "0123456789.")
        }
        
        //拒绝输入空和小说点
        if (inputText as NSString).isEqualToString("") && (inputText as NSString).isEqualToString(".") {
            return false
        }
        
        let tempStr = inputText.stringByAppendingString(replaceString)
        let strlen = (tempStr as NSString).length
        
        if pointRange.length > 0 && pointRange.location > 0 { //判断输入框是否含有"."
            if (replaceString as NSString).isEqualToString(".") {  //当输入框已含有"."时,如果再输入".",则视为无效
                return false
            }
        ...
        let zeroRange:NSRange = (inputText as NSString).rangeOfString("0")
        if zeroRange.length == 1 && zeroRange.location == 0 { //判断输入框第一个字符是否为"0"
            if !(replaceString as NSString).isEqualToString("0") && !(replaceString as NSString).isEqualToString(".") { //当输入框只有一个字符并且字符为“0”时，再输入不为“0”或者“.”的字符时，则将此输入替换输入框的这唯一字符。
                ...
                if pointRange.length == 0 && pointRange.location > 0 { ////当输入框第一个字符为“0”时，并且没有“.”字符时，如果当此输入的字符为“0”，则视当此输入无效。
                    ...
                    if (replaceString as NSString).isEqualToString("0") {
                        return false
                    }
                }
            }
        ...
        var  buffer:NSString?
        if !scanner.scanCharactersFromSet(numbers, intoString: &buffer) && (replaceString as NSString).length != 0 {
            return false
        }
