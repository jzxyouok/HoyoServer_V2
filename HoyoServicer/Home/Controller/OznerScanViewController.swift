...
//    }
    ...
    override func handleCodeResult(arrayResult: [LBXScanResult]) {
        ...
        delegate?.popToSubmitCon(result.strScanned!)
...
//        navigationController?.pushViewController(vc, animated: true)
    ...
    func drawBottomItems()
    {
        ...
        btnFlash.center = CGPointMake(CGRectGetWidth(bottomItemsView!.frame)*3/4, CGRectGetHeight(bottomItemsView!.frame)/2)
        btnFlash.setImage(UIImage(named: "CodeScan.bundle/qrcode_scan_btn_flash_nor"), forState:UIControlState.Normal)
        btnFlash.addTarget(self, action: #selector(OznerScanViewController.openOrCloseFlash), forControlEvents: UIControlEvents.TouchUpInside)
