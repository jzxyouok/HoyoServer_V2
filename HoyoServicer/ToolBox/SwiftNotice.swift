    ...
    static func showNoticeWithText(type: NoticeType,text: String, autoClear: Bool, autoClearTime: Int) {
        ...
        var image = UIImage()
            ...
            didSet{
                    ...
                    label.frame.size = CGSize(width:  UIScreen.mainScreen().bounds.size.width/2, height: label.frame.height)
                     label.sizeToFit()
...
//        let frame = CGRectMake(0, 0, label.frame.size.width + 20, label.frame.height + )
        let window = UIWindow()
        window.backgroundColor = UIColor.clearColor()
        let mainView = UIView()
...
//            {
//            didSet{
//            if image == SwiftNoticeSDK.imageOfCross
//            {
//                label.frame.size = CGSize(width:  UIScreen.mainScreen().bounds.size.width/2, height: label.frame.height)
//                
//                }
...
//        mainView.addSubview(checkmarkView)
       ...
       //    textLabel.frame = CGRectMake(20, 70, expectSize.width, expectSize.height);
        ...
        let frame = CGRectMake(0, 0, label.frame.size.width + 30, label.frame.height + checkmarkView.frame.height*2.3 )
      ...
      //  label.text = text
        ...
        center.x = mainView.frame.width/2
        center.y = mainView.frame.height*2/3
        label.center = center
      var center2 = checkmarkView.center
        center2.x = mainView.frame.width/2
        center2.y = checkmarkView.frame.height/2 + mainView.frame.size.height/7
        checkmarkView.center = center2
        mainView.addSubview(checkmarkView)
        window.windowLevel = UIWindowLevelAlert
        window.center = getRealCenter()
        // change orientation
