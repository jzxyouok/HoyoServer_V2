...
class LoginAndRegisterViewController: UIViewController,UITextFieldDelegate {
    ...
    func buttonClick(button:UIButton){
        ...
        case 7://注册页面三：下一步
            ...
            MBProgressHUD.showHUDAddedTo(self.view, animated: true)
            User.AppRegister(TmpToken!,realname: tmpName!, cardid: tmpIDCard!, password: tmppassWord!, success: { [weak self](user) in
                MBProgressHUD.hideHUDForView(self!.view, animated: true)
                User.currentUser = user
    ...
    func textFieldShouldReturn(textField: UITextField) -> Bool {
        ...
        if textField != registFooterView3?.invitationCodeTextField {
            ...
            if textField == registFooterView1?.phoneTextField || textField == registFooterView2?.codeTextField {
